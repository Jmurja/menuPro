        ...
        <div class="relative w-full max-w-xl max-h-full mx-auto mt-20">
            <div class="relative bg-white rounded-lg shadow dark:bg-zinc-800">
                ...
                <form action="{{ route('users.update', $user) }}" method="POST" class="p-6 space-y-6">
                    ...
                    <div>
                        <label for="name-{{ $user->id }}" class="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Nome</label>
                    ...
                    <div>
                        <label for="email-{{ $user->id }}" class="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Email</label>
                        <input type="email" name="email" id="email-{{ $user->id }}" value="{{ $user->email }}" required
                               class="mt-1 block w-full rounded-md border-zinc-300 shadow-sm dark:bg-zinc-700 dark:border-zinc-600 dark:text-white">
                    </div>

                    <div>
                        <label for="role-{{ $user->id }}" class="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Cargo</label>
                        <select name="role" id="role-{{ $user->id }}"
                                class="mt-1 block w-full rounded-md border-zinc-300 shadow-sm dark:bg-zinc-700 dark:border-zinc-600 dark:text-white">
                            @foreach(\App\Enums\UserRole::options() as $option)
                                <option value="{{ $option['value'] }}" @selected($user->role->value === $option['value'])>
                                    {{ $option['label'] }}
                                </option>
                            @endforeach
                        </select>
